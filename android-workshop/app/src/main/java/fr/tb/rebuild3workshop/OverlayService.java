...
public class OverlayService extends Service {
    ...
    private final BroadcastReceiver progressReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            int partPercent = Math.min(100, Math.max(0, (int)((current * 100L) / total)));
            int overallPercent = (int)Math.min(
                100,
                Math.max(
                    0,
                    (((part - 1) + (partPercent / 100.0)) / partTotal) * 100.0
                )
            );

            if (titleView != null) {
                titleView.setText("Rebuild Workshop  •  " + part + "/" + partTotal);
            }
            if (detailView != null) {
                detailView.setText(
                    status + "\n" +
    ...
    public void onCreate() {
        ...
        IntentFilter filter = new IntentFilter(AppState.ACTION_PROGRESS);
    ...
    private void createOverlay() {
        ...
        background.setStroke(dp(1), Color.argb(190, 100, 100, 100));
        ...
        detailView.setText("Prêt. Ouvre Install Mod et utilise le clavier Workshop.");
        ...
        close.setText("Fermer l'overlay");
    ...
    private void createNotificationChannel() {
        ...
        channel.setDescription("Affiche la progression d'installation au-dessus de Rebuild 3.");
